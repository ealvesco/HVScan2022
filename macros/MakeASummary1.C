...
void MakeASummary1(const char* subdetect, bool BL=false){
  ...
  double id;
  //if (BL)blacklistrollv = blacklist("../data/blacklist_2018b.txt"); 
  ...
  /*TString outputFile;
  if (strcmp(subdetect,"barrel" ) ==0 && BL==true )outputFile  = "/afs/cern.ch/work/r/rreyesal/public/RPC/HVScanSW/ComparisonAge/summary/barrel_summary_Exc2018BlackList.root";
  else if (strcmp(subdetect,"endcap")==0 && BL==true ) outputFile = "/afs/cern.ch/work/r/rreyesal/public/RPC/HVScanSW/ComparisonAge/summary/endcap_summary_Exc2018BlackList.root";
...
*/
  ...
  std::string name_;
  std::string id_;
  Double_t FitResEff[12];
  Double_t FitResCls[8];
 
  if (BL){
	if (strcmp(subdetect,"barrel")==0){
					  rollremoved.open("../summary/removed_barrel_May2018.txt");
					  rollData.open("../summary/filtered_barrel_May2018.txt");
					  }
	else {rollremoved.open("../summary/removed_endcap_May2018.txt"); rollData.open("../summary/filtered_endcap_May2018.txt");}
 	}
  else{
   if (strcmp(subdetect,"barrel")==0)rollData.open("../summary/summary_barrel_2018.txt");
   else {rollData.open("../summary/summary_endcap_2018.txt");}
  }	
  ...
  for (vector<std::pair<std::string,std::string> >::const_iterator itmap = map.begin() ;itmap != map.end(); itmap++  ){
  	 ...
  	 id_ =itmap->second;
         ...
         while (fResEff.good()){
          ...
          for (vector<std::pair<std::string, double> >::const_iterator itchannel = channel_map.begin() ; itchannel != channel_map.end(); itchannel++ ){
                 if(strcmp(subdetect,"barrel")==0 ){
			if ( (itmap->first).find(itchannel->first) != std::string::npos){
			FitResEff[10] = ((itchannel->second)/1000.0);
			FitResCls[7] = ((itchannel->second)/1000.0);
	  	    	FitResEff[11] = Sigmoidcalc( FitResEff[10] , FitResEff[2], FitResEff[9], FitResEff[3]); 
			std::cout << itchannel->first << "  " <<itmap->first << "  " << FitResEff[10] << "  " << FitResEff[11] << "  " <<  std::endl;
			//continue;   

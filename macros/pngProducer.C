...
//  c1->SetGridy(1); 
 ...
 hveff->GetXaxis()->SetTitleSize(0.06);
 hveff->GetXaxis()->SetTitleOffset(1.15);
 hveff->GetYaxis()->SetTitleSize(0.06);
 hveff->GetYaxis()->SetTitleOffset(1.1);
 hveff->Draw("AP");
 ...
 c1->SaveAs(("../results/"+id_+"_EFFvsHV.C").c_str());
 c1->Clear(); 
 return; 
}
 ...
 hvcls->GetXaxis()->SetTitleSize(0.06);
 hvcls->GetXaxis()->SetTitleOffset(1.15);
 hvcls->GetYaxis()->SetTitleSize(0.06);
...
void pngProducer(const char* subdetect){
   ...
   {      
      ...
      while (1){
		  cout << itmap->first << "  " << FitResEff[2] << endl;
